...
- При **проигрыше/ничьей**: «Сыграть ещё раз» доступно **до 3 попыток**.
...
- `TELEGRAM_CHAT_ID=<ваш chat_id>` (если хотите отправлять в один тестовый/админский чат вместо привязки игроков)

## Хранение промокодов

Выданные промокоды логируются на сервере в файл `server/data/promocodes.jsonl` (создаётся автоматически).
